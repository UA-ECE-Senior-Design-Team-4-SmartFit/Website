...
echo "$First_Name $Last_Name";
 ?>
        ...
        <?php
        $query=  "SELECT `Trainer_FName`,`Trainer_LName`,`Age`,`Height`,`Weight`,`Sex`,`Email` FROM `Trainer_Data` WHERE `Trainer_ID`=1";
        $result = mysqli_query($con,$query) or die('Query failed: ' . mysqli_error());
      ...
      <div class="thumbnail">
        <a href="/w3images/nature.jpg" target="_blank">
          <img src="https://www.bodybuilding.com/fun/images/2013/4-keys-to-surviving-as-a-trainer-1.jpg" alt="Nature" style="width:100%">
            ...
            <?php
            $query=  "SELECT `User_FName`,`User_LName`,`Age`,`Weight`,`Height`,`Sex`,`Email` FROM `User_Data` WHERE `Trainer_ID`=1";
            $result = mysqli_query($con,$query) or die('Query failed: ' . mysqli_error());
            // Printing results in HTML
            $Has_Value=true;
          if (mysqli_num_rows($result) <= 0) {
